...
//( sound , fermi , coll , visco , odd_visco , cyclo , thermal_diff , mode , aspect )
...
teste.SetSound([](float x) { return 1.0f+0.5f*tanh(8.0f*cos(5.0f*2.0f*MAT_PI*x)); });
//teste.SetDt();
//teste.SetSound();
...
string ext = ".dat";
	...
	for (float h = 0.0f; h < 2.5f ; h+=teste.GetDt()) {
		...
		DirichletBoundaryCondition::VelocityX(teste,0.5f,0.5f);
		//BoundaryCondition::XFree(teste);



		if(GrapheneFluid1D::TimeStepCounter % 50 == 0){
			name = name+"_"+to_string(GrapheneFluid1D::TimeStepCounter/50)+ext;
			ofstream outputfile;
			outputfile.open (name);
			for (int i = 0; i < teste.SizeX(); ++i) {
				outputfile << i*teste.GetDx() << "\t" <<  teste.Umain[i] <<endl;
			}
			outputfile.close();
			name = "space";
		}
	...
	outputfile1.open ("time.dat");
	for (float h = 0.0f; h < 2.5f ; h+=teste.GetDt()) {
		outputfile1 << h << "\t" <<  teste.Umain[q] <<endl;
		q++;
	}
